...
def euclidean_distances(X, Y):
    distances = X[:,None,:].astype(float) - Y[None,:,:].astype(float)
...
def init_centroids(num_clusters, image):
    ...
    # *** START YOUR CODE ***
    H, W, C = image.shape
    indices = np.random.choice(np.arange(H * W), num_clusters, replace=False)
    centroids_init = image.reshape((H * W, C))[indices]
    # *** END YOUR CODE ***
...
def update_centroids(centroids, image, max_iter=30, print_every=10):
    ...
    for _ in range(max_iter):
        centroid_distances = euclidean_distances(image, centroids)
        closest_centroid_index = np.argmin(centroid_distances, axis=1)
        for j in range(N):
            centroids[j] = image[closest_centroid_index == j].mean(axis=0)    
...
def update_image(image, centroids):
    ...
    centroid_distances = euclidean_distances(image, centroids)
    closest_centroid_index = np.argmin(centroid_distances, axis=1)
    image[:] = centroids[closest_centroid_index]
    image = image.reshape((H, W, C))
    # *** END YOUR CODE ***
