...
def initial_state():
    ...
    model_state ={}
    model_state['beta']=[0.0]
    initial_x_0 = np.ones(2)
    model_state['training_x'] = [initial_x_0] ## taking 1 for the initial value
    return model_state
...
def predict(state, kernel, x_i):
    ...
    # *** START CODE HERE ***
    sum = 0
    for beta_l, x_l in zip(state['beta'], state['training_x']):
        sum += beta_l * kernel(x_l, x_i)
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    beta_new = learning_rate * (y_i - sign(sum))

    state['beta'].append(beta_new)
    state['training_x'].append(x_i)
    # *** END CODE HERE ***
