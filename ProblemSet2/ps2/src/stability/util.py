...
def plot(x, y, theta, save_path, correction=1.0):
    ...
    # Plot dataset
    plt.figure()
    plt.plot(x[y == 1, -2], x[y == 1, -1], 'bo', linewidth=0)
    plt.plot(x[y == 0, -2], x[y == 0, -1], 'gx', linewidth=0)
...
def plot_points(x, y):
    ...
    plt.scatter(x_two[:,0], x_two[:,1], marker='o', color='green')

def plot_points_fig(x, y,save_path):
    """Plot some points where x are the coordinates and y is the label"""
    x_one = x[y == 0, :]
    x_two = x[y == 1, :]
    ...
    # print(x_one)
    plt.figure()
    plt.plot(x_one[:,1], x_one[:,2], 'x', color='red')
    plt.plot(x_two[:,1], x_two[:,2], 'o', color='blue')
...
def plot_accuracy_curve(data, save_path):
    """Plot some points where x are the coordinates and y is the label"""
