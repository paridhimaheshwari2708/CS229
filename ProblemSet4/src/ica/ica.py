...
def update_W(W, x, learning_rate):
    ...
    # *** START CODE HERE ***
    x= x.reshape(-1,1) ## Reshaping it to a vector of shape [N,1]
    W_inv_transpose = LA.inv(W).transpose()
    second_part_a = np.sign(np.matmul(W,x))
    second_part_b = x.transpose()
    second_part = np.matmul(second_part_a, second_part_b) 
    updated_W = W + learning_rate * (W_inv_transpose - second_part)

    # print("Updated W", updated_W.shape)
    # *** END CODE HERE ***
